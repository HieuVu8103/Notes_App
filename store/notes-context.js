...
export const NotesContext = createContext({
    ...
    deleteNote: (id) => {},
    updateNote: (id, {color, labelIds, content, updateAt, isBookmarked}) => {},
});
...
function notesReducer(state, action) {
    switch (action.type) {
        ...
        case 'UPDATE':
            const updatableNoteindex = state.findIndex((note) => note.id === action.payload.id);
            const updatableNote = state[updatableNoteindex];
            const updatedItem = { ...updatableNote, ...action.payload.data };
